...
> `ng test --no-watch --coverage` → **37 test files · 456 specs · 0 fallos · 88.94% statements**
> (85.66% branches · 81.69% functions · 93.78% lines, ≥70% ✅); `mvn test` → **405 tests · 0 fallos ·
> BUILD SUCCESS**. **Módulo Inventario CERTIFICADO bajo Propuesta D.** Pendiente: commit `chore(qa)`.
> Ronda anterior completada el 2026-06-11: 191 ✅ PASS, 0 ❌ FAIL, 7 N/A.
...
|---|---|---|---|---|---|---|
| SEC-01 | Sin sesión — acceso directo a `/inventory/products` | (sin JWT) | Token expirado / sin login | Redirige a `/login` | ✅ PASS | Fase 1 Ronda 6 (2026-06-26): `localStorage.clear()` + navegar a `/inventory/products` → `location.pathname === /login`. Re-test 2026-06-10 igual. |
| SEC-02 | Sin sesión — acceso directo a `/inventory/categories` | (sin JWT) | Token expirado / sin login | Redirige a `/login` | ✅ PASS | Fase 1 Ronda 6 (2026-06-26): sin token, navegar a `/inventory/categories` → `pathname === /login`. Re-test 2026-06-10 igual. |
| SEC-03 | Sin sesión — acceso directo a `/inventory/low-stock` | (sin JWT) | Token expirado / sin login | Redirige a `/login` | ✅ PASS | Fase 1 Ronda 6 (2026-06-26): sin token, navegar a `/inventory/low-stock` → `pathname === /login`. Re-test 2026-06-10 igual. |
| SEC-04 | SALES accede a `/inventory/products` — lectura permitida | SALES | Sesión SALES activa | Carga la página; lista visible; sin botón "Nuevo producto" | ✅ PASS | Fase 1 Ronda 6 (2026-06-26): login `qa_sales` (L35) — chip topbar "qa_sales \| SALES" (naranja), sin redirect (`pathname === /inventory/products`), 20 filas visibles, `button "Nuevo producto"` AUSENTE del DOM, sin snackbar de error, filtros Buscar/Categoría/Estado presentes, filtro "Proveedor" AUSENTE (rol sin acceso a `/purchases/**`). Re-test 2026-06-12 igual. |
| SEC-05 | SALES accede a `/inventory/categories` — lectura permitida | SALES | Sesión SALES activa | Carga la página; lista visible; sin botón "Nueva categoría" | ✅ PASS | Fase 1 Ronda 6 (2026-06-26): login `qa_sales` — chip "SALES", sin redirect, 20 filas visibles, `button "Nueva categoría"` AUSENTE del DOM, sin snackbar de error. Obs (no defecto): categoría stale `<script>document.title="XSS-CAT-CYBER05"</script>QA-XSS-20` se renderiza como TEXTO LITERAL (XSS no ejecuta) — dato QA por limpiar (L33). Re-test 2026-06-10 igual. |
...
|---|---|---|---|---|---|
| VIS-PROD-01 | Título "Productos" visible | ADMIN | Encabezado "Productos" presente | ✅ PASS | Fase 1 R6 (2026-06-26): breadcrumb "Inventario → Productos" presente en la topbar. |
| VIS-PROD-02 | Columnas tabla: SKU, Nombre, Categoría, Proveedor, Stock, Precio, Estado | ADMIN | Todas las columnas visibles | ✅ PASS | Fase 1 R6 (2026-06-26): headers Código/Nombre/Categoría/Proveedor/Stock/Precio/Costo unit./Estado. |
| VIS-PROD-03 | Columna "Costo unitario" visible para ADMIN | ADMIN | Columna presente en la tabla | ✅ PASS | Fase 1 R6 (2026-06-26): columna "Costo unit." presente con valores (ADMIN). |
| VIS-PROD-04 | Columna "Acciones" visible para WAREHOUSEMAN | WAREHOUSEMAN | Ícono de movimiento en cada fila | ✅ PASS | Fase 1 R6 (2026-06-26): `qa_warehouse` — ícono `swap_vert` por fila (RBAC-PROD-10 esta ronda). |
| VIS-PROD-05 | Barra de búsqueda y filtros visibles | ADMIN | Campos búsqueda, categoría, estado, proveedor presentes | ✅ PASS | Fase 1 R6 (2026-06-26): Buscar/Categoría/Estado/Proveedor presentes. |
| VIS-PROD-06 | `StockBadge` muestra colores semánticos | ADMIN | Verde (ok), naranja (bajo), rojo (sin stock) | ✅ PASS | Fase 1 R6 (2026-06-26): `getComputedStyle` badges con stock OK → verde bg `rgb(232,245,233)` (#E8F5E9) texto `rgb(46,125,50)` (#2E7D32). Naranja/rojo son el mismo `StockBadgeComponent` por umbral (RGB documentado ronda previa). |
| VIS-PROD-07 | `mat-progress-bar` visible durante carga | ADMIN | Barra en parte superior mientras carga | ✅ PASS | Fase 1 R6 (2026-06-26): `@if (loading) { <mat-progress-bar mode=indeterminate> }` en template (code review; carga demasiado rápida para captura visual). |
...
| BSRCH-PROD-08 | Botón "Limpiar filtros" restaura la lista completa | ADMIN | Filtros activos | Lista sin filtros; contador de filtros activos = 0 | ✅ PASS | Fase 1 R6 (2026-06-26): con filtro Estado activo, click `filter_alt_off` ("Limpiar todos los filtros") → lista completa restaurada "1–20 of 191", botón limpiar desaparece. |

### 1c. RBAC en lista (RBAC)

| ID | Descripción | Rol | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| RBAC-PROD-01 | Botón "Nuevo producto" visible | ADMIN | Visible | ✅ PASS | Fase 1 R6 (2026-06-26): `qa_manager`→no, `admin` chip ADMIN, `button "Nuevo producto"` presente (DOM). |
| RBAC-PROD-02 | Botón "Nuevo producto" visible | MANAGER | Visible | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_manager` (L35) — chip MANAGER, `button "Nuevo producto"` presente (DOM). |
| RBAC-PROD-03 | Botón "Nuevo producto" AUSENTE | WAREHOUSEMAN | Ausente del DOM | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_warehouse` — chip WAREHOUSEMAN, `button "Nuevo producto"` AUSENTE del DOM. |
| RBAC-PROD-04 | Botón "Nuevo producto" AUSENTE | SALES | Ausente del DOM | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_sales` — chip SALES, `button "Nuevo producto"` AUSENTE del DOM. |
| RBAC-PROD-05 | Columna "Costo unitario" visible | ADMIN | Columna presente | ✅ PASS | Fase 1 R6 (2026-06-26): headers incluyen "Costo unit." con valores (ADMIN). |
| RBAC-PROD-06 | Columna "Costo unitario" visible | MANAGER | Columna presente | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_manager` — headers incluyen "Costo unit." con valores. |
| RBAC-PROD-07 | Columna "Costo unitario" AUSENTE del DOM | WAREHOUSEMAN | Columna no renderizada | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_warehouse` — headers Código/Nombre/Categoría/Proveedor/Stock/Precio/Estado, sin "Costo unit." |
| RBAC-PROD-08 | Columna "Costo unitario" AUSENTE del DOM | SALES | Columna no renderizada | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_sales` — headers Código/Nombre/Categoría/Proveedor/Stock/Precio/Estado, sin "Costo unit." |
| RBAC-PROD-09 | Ícono "Registrar movimiento" visible | ADMIN | Ícono en cada fila | ✅ PASS | Fase 1 R6 (2026-06-26): ícono `swap_vert` presente en cada fila (20 filas → 20 íconos). |
| RBAC-PROD-10 | Ícono "Registrar movimiento" visible | WAREHOUSEMAN | Ícono en cada fila | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_warehouse` — `swap_vert` en cada fila (20/20). |
| RBAC-PROD-11 | Ícono "Registrar movimiento" AUSENTE | SALES | Columna acciones sin ícono | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_sales` — sin columna "Acciones", 0 íconos `swap_vert` (displayedColumns sin 'actions'). |
...
| ID | Descripción | Rol | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|---|
| UI-PROD-01 | Click en fila abre diálogo de detalle | ADMIN | Lista cargada | Diálogo `ProductDetailDialog` abre sin navegar | ✅ PASS | Fase 1 R6 (2026-06-26): click fila → "Editar producto" sin cambiar URL (verificado en RBAC-PDET-02/CRUD-PDET-02 esta ronda). |
| UI-PROD-02 | Click en fila (WAREHOUSEMAN) abre diálogo en modo lectura | WAREHOUSEMAN | Lista cargada | Diálogo abre; campos deshabilitados; sin botón Guardar | ✅ PASS | Fase 1 R6 (2026-06-26): `qa_warehouse` → "Ver producto" campos como texto, único botón "Cerrar" (verificado en RBAC-PDET-03/05 esta ronda). |
| UI-PROD-03 | Ícono movimiento → clic abre `MovementDialog` sin navegar | ADMIN | Lista cargada | Diálogo de movimiento abre; página permanece | ✅ PASS | Fase 1 R6 (2026-06-26): `swap_vert` → "Registrar movimiento de stock" con Stock físico/Reservado/Disponible (CRUD-MOV esta ronda). |
| UI-PROD-04 | Ícono movimiento → confirmar movimiento → stock se actualiza | ADMIN | MovementDialog abierto con tipo IN | Snackbar verde; lista recarga con nuevo stock | ✅ PASS | Fase 1 R6 (2026-06-26): IN +5 sobre HMAN-DES-013 → snackbar verde, stock 102→107 (CRUD-MOV-01 esta ronda). |
| UI-PROD-05 | Ícono movimiento → cancelar → stock NO cambia | ADMIN | MovementDialog abierto | Cerrar sin guardar; stock sin cambios | ✅ PASS | Fase 1 R6 (2026-06-26): Cancelar con datos llenados → stock HMAN-DES-013 sin cambios (CRUD-MOV-03 esta ronda). |
| UI-PROD-PAG-01 | Paginador visible cuando hay > pageSize productos | BD con >20 productos | Paginador con total y opciones 10/20/50 | ✅ PASS | Fase 1 R6 (2026-06-26): "1 – 20 of 191", selector "Items per page" con opciones 10/20/50. |
| UI-PROD-PAG-02 | Cambio de página carga página siguiente | Paginador visible | Filas cambian al ir a pág 2 | ✅ PASS | Fase 1 R6 (2026-06-26): "siguiente" → "21 – 40 of 191" con filas distintas (HMAN-JLL-012, COMP-LT15-003, BLAN-LAV17-009). |
| UI-PROD-PAG-03 | Cambiar tamaño de página (10/20/50) recarga la lista con el nuevo tamaño | Paginador visible, >50 productos | Selector "Items por página" cambia a 10/50; tabla muestra el número correcto de filas; vuelve a página 0 | ✅ PASS | Fase 1 R6 (2026-06-26, BUG-INV-10 sigue corregido): en página 3 ("41-60 of 191", size=20), cambiar "Items per page" a 10 → "1 – 10 of 191" (página 0), 10 filas. |

### 1e. Estados vacíos (EMPTY)

| ID | Descripción | Resultado esperado | Estado | Notas |
|---|---|---|---|---|
| EMPTY-PROD-01 | Búsqueda sin resultados | Ícono + 'Sin resultados para "…"' | ✅ PASS | Fase 1 R6 (2026-06-26): "zzz999"/"zzz999noexiste" → 0 filas + empty-state "Sin resultados / Ningún registro coincide con los filtros aplicados." (BSRCH-PROD-07 esta ronda). |
| EMPTY-PROD-02 | Filtro activo sin resultados | Mensaje + botón "Limpiar filtros" visible | ✅ PASS | Fase 1 R6 (2026-06-26): con búsqueda "zzz999noexiste" activa (0 resultados) → empty-state + botón `filter_alt_off` ("Limpiar todos los filtros") visible. |
...
| ID | Descripción | Rol | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| RBAC-PDET-01 | Título "Nuevo producto" al crear | ADMIN | "Nuevo producto" como título | ✅ PASS | Fase 1 R6 (2026-06-26): diálogo "Nuevo producto" (tabs Datos/Kardex) con SKU/Nombre/Precio/Costo unitario/Stock inicial/Stock mínimo/Estado/Categoría/Proveedor/Descripción. |
| RBAC-PDET-02 | Título "Editar producto" al editar | ADMIN | "Editar producto" como título | ✅ PASS | Fase 1 R6 (2026-06-26): click fila HMAN-DES-013 → diálogo "Editar producto". |
| RBAC-PDET-03 | Título "Ver producto" para WAREHOUSEMAN | WAREHOUSEMAN | "Ver producto" como título | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_warehouse` — click fila → título "Ver producto". |
| RBAC-PDET-04 | Campos editables y botón Guardar visible | ADMIN | Todos los inputs habilitados; "Guardar" presente | ✅ PASS | Fase 1 R6 (2026-06-26): nuevo→6 inputs editables (0 disabled); editar→"Guardar cambios" presente, 5 editables + SKU read-only (clave inmutable, esperado). |
| RBAC-PDET-05 | Campos deshabilitados y sin botón Guardar | WAREHOUSEMAN | inputs `disabled:true`; sin "Guardar" | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_warehouse` — "Ver producto" muestra datos como texto (0 inputs editables), único botón "Cerrar", sin "Guardar". |
| RBAC-PDET-06 | Botón "Desactivar" visible solo para ADMIN | ADMIN | Visible | ✅ PASS | Fase 1 R6 (2026-06-26): diálogo edición → botones Desactivar/Cancelar/Guardar cambios. |
| RBAC-PDET-07 | Botón "Desactivar" AUSENTE para MANAGER | MANAGER | Ausente del DOM | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_manager` — "Editar producto" con botones solo Cancelar/Guardar cambios, sin "Desactivar". |
| RBAC-PDET-08 | Campo "Costo unitario" visible para ADMIN | ADMIN | Campo presente y editable | ✅ PASS | Fase 1 R6 (2026-06-26): label "Costo unitario" presente y editable (input no disabled) en nuevo y editar. |
| RBAC-PDET-09 | Campo "Costo unitario" AUSENTE para WAREHOUSEMAN | WAREHOUSEMAN | Campo no renderizado | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_warehouse` — "Ver producto" sin campo/label "Costo unitario". |
...
|---|---|---|---|---|---|
| VAL-PDET-01 | Campo SKU vacío → error inline | Formulario nuevo | Error "SKU requerido" bajo el campo | ✅ PASS | Fase 1 R6 (2026-06-26): SKU vacío + touch/blur → `mat-error` "El SKU es obligatorio." |
| VAL-PDET-02 | Campo Nombre vacío → error inline | Formulario nuevo | Error "Nombre requerido" | ✅ PASS | Fase 1 R6 (2026-06-26): Nombre vacío + blur → "El nombre es obligatorio." |
| VAL-PDET-03 | Precio ≤ 0 → error | Precio = 0 | Error "Debe ser mayor a cero" | ✅ PASS | Fase 1 R6 (2026-06-26): Precio=0 + blur → "El precio debe ser mayor a 0." |
| VAL-PDET-04 | Stock mínimo negativo → error | Valor negativo | Error de validación visible | ✅ PASS | Fase 1 R6 (2026-06-26): Stock mínimo=-1 + blur → "El stock mínimo no puede ser negativo." (BUG-INV-02 sigue corregido) |
| VAL-PDET-05 | Formulario inválido → botón Guardar deshabilitado | Campos inválidos | `disabled:true` en el DOM | ✅ PASS | Fase 1 R6 (2026-06-26): formulario nuevo vacío → "Crear producto" `disabled:true`. |
| VAL-PDET-06 | Formulario de edición recién cargado → Guardar deshabilitado | Sin modificaciones | `disabled:true` (form.dirty = false) | ✅ PASS | Fase 1 R6 (2026-06-26): abrir "Editar producto" HMAN-DES-013 → "Guardar cambios" `disabled:true` (pristine). |
| VAL-PDET-07 | Modificar un campo → Guardar se activa | Modificar nombre | `disabled:false` | ✅ PASS | Fase 1 R6 (2026-06-26): teclear en Nombre (tecleo real) → "Guardar cambios" `disabled:false` (L25 dirty). |
| VAL-PDET-08 | Guardar exitoso → Guardar se desactiva | Guardar con éxito | `disabled:true` (markAsPristine) | ✅ PASS | Fase 1 R6 (2026-06-26): tras guardar (CRUD-PDET-02), al reabrir edición "Guardar cambios" `disabled:true` (pristine), se reactiva solo al modificar (corroborado VAL-PDET-06/07). |
| VAL-PDET-09 | SKU duplicado → snackbar error con mensaje del backend | SKU ya existe | Snackbar rojo con mensaje 409/422 del backend | ✅ PASS | Fase 1 R6 (2026-06-26): editar HMAN-DES-013 → SKU "LUBR-ACE20-035" (en uso) → Guardar → snackbar rojo `rgb(198,40,40)` `snackbar-error` "El SKU 'LUBR-ACE20-035' ya está en uso por otro producto.", diálogo permanece abierto. Cancelado sin guardar (HMAN intacto). |
| VAL-PDET-10 | Campo Stock no es editable en el formulario | Formulario de edición | Campo "Stock actual" ausente o `disabled:true` — se registra vía movimientos | ✅ PASS | Fase 1 R6 (2026-06-26): en edición, campo "Stock físico (solo lectura)" `disabled:true` + ícono candado + hint "El stock físico solo puede modificarse mediante Registrar movimiento." |
| VAL-PDET-11 | Campo SKU > 50 caracteres → error o input no acepta más | Escribir 51+ caracteres en SKU | Error "Máximo 50 caracteres" o `maxlength` en el DOM impide escribir más | ✅ PASS | Fase 1 R6 (2026-06-26): atributo `maxlength=50` confirmado en input SKU vía DOM. |
| VAL-PDET-12 | Campo Nombre > 150 caracteres → error o input no acepta más | Escribir 151+ caracteres en Nombre | Error "Máximo 150 caracteres" o `maxlength` en el DOM impide escribir más | ✅ PASS | Fase 1 R6 (2026-06-26): atributo `maxlength=150` confirmado en input Nombre vía DOM. |
| VAL-PDET-13 | Campo Descripción > 500 caracteres → error o input no acepta más | Escribir 501+ caracteres en Descripción | Error "Máximo 500 caracteres" o `maxlength` en el DOM impide escribir más | ✅ PASS | Fase 1 R6 (2026-06-26): atributo `maxlength=500` confirmado en textarea Descripción vía DOM. |
| VAL-PDET-14 | Guardar sin seleccionar Categoría → formulario inválido | Formulario nuevo, categoría sin seleccionar, resto completo | Botón "Guardar" deshabilitado o error "Categoría requerida" | ✅ PASS | Fase 1 R6 (2026-06-26): Categoría sin seleccionar + blur → "La categoría es obligatoria." (concordancia femenina CORRECTA, BUG-INV-08 sigue corregido) + botón deshabilitado. |
| VAL-PDET-15 | Guardar sin seleccionar Proveedor → formulario inválido | Formulario nuevo, proveedor sin seleccionar, resto completo | Botón "Guardar" deshabilitado o error "Proveedor requerido" | ✅ PASS | Fase 1 R6 (2026-06-26): Proveedor sin seleccionar + blur → "El proveedor es obligatorio." + botón deshabilitado. |

### 2c. CRUD Productos (CRUD)

| ID | Descripción | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| CRUD-PDET-01 | Crear producto con todos los datos válidos | Formulario nuevo completo | Snackbar verde "Producto creado correctamente."; diálogo cierra; lista recarga | ✅ PASS | Fase 1 R6 (2026-06-26): creado `QA-CRUD-R6-001` "[QA] Producto CRUD Ronda6" (precio $100, costo $60, stock 50, mín 5, cat Cat-Cy-18832, prov Agroquímica…) → snackbar verde `rgb(46,125,50)` `snackbar-success` "Producto creado correctamente.", diálogo cierra, búsqueda lo muestra en la lista. |
| CRUD-PDET-02 | Editar nombre de un producto | Producto existente | Snackbar verde "Producto actualizado."; lista refleja cambio | ✅ PASS | Fase 1 R6 (2026-06-26): editado nombre → "[QA] Producto CRUD R6 editado" (tecleo real activa Guardar por `form.dirty`, L25) → snackbar verde "Producto actualizado correctamente.", diálogo cierra. |
| CRUD-PDET-03 | Desactivar producto — click Desactivar abre diálogo de confirmación | ADMIN, producto activo | Modal con texto confirmatorio | ✅ PASS | Fase 1 R6 (2026-06-26): click "Desactivar" dentro del diálogo edición abre modal "Desactivar producto" con texto '¿Deseas desactivar "[QA] Producto CRUD R6 editado" (QA-CRUD-R6-001)?' + botones Cancelar/Desactivar (`disableClose`, L31). |
| CRUD-PDET-04 | Cancelar desactivación — producto permanece activo | Diálogo confirmación | Sin cambios | ✅ PASS | Fase 1 R6 (2026-06-26): "Cancelar" cierra el modal (dialogCount 2→1), el diálogo de edición permanece abierto y Estado sigue "Disponible". |
| CRUD-PDET-05 | Confirmar desactivación — producto desaparece de la lista activa | ADMIN, confirmar | Snackbar verde; producto ya no aparece | ✅ PASS | Fase 1 R6 (2026-06-26): "Desactivar" confirma; búsqueda "QA-CRUD-R6-001" → empty-state "Sin resultados / Ningún registro coincide…" (filtro por defecto = activos). Limpieza L33: producto QA desactivado. |

### 2d. Kardex / Historial de movimientos (UI)

| ID | Descripción | Rol | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|---|
| UI-PDET-01 | Sección "Kardex" visible en diálogo de detalle | ADMIN | Producto con movimientos | Tabla de movimientos visible con fecha, tipo, cantidad, motivo | ✅ PASS | Fase 1 R6 (2026-06-26): tab "Kardex" de HMAN-DES-013 muestra tabla con columnas Fecha/Tipo/Cantidad/Motivo/Usuario; 10 filas; 1ª = "26/06/2026 09:24, Salida, -3, [QA] Salida prueba CRUD-MOV-02 Ronda6, admin". |
| UI-PDET-02 | Sección Kardex visible para WAREHOUSEMAN | WAREHOUSEMAN | Producto con movimientos | Tabla visible | ✅ PASS | Fase 1 R6 (2026-06-26): el diálogo "Ver producto" de `qa_warehouse` contiene tabs "Datos"/"Kardex" (verificado en RBAC-PDET-03 esta ronda) → Kardex accesible en modo lectura. |
| UI-PDET-03 | Paginación del Kardex funciona | ADMIN | Producto con movimientos | Paginador activo; cambio de página carga filas siguientes | ✅ PASS | Fase 1 R6 (2026-06-26): Kardex HMAN-DES-013 "1 – 10 of 20"; "siguiente" → "11 – 20 of 20", 10 filas. |
...
| ID | Descripción | Rol | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| VIS-MOV-01 | Diálogo muestra nombre y SKU del producto | ADMIN | Nombre y SKU del producto seleccionado visibles | ✅ PASS | Fase 1 R6 (2026-06-26): "Registrar movimiento de stock" muestra "Producto: (editado QA) (HMAN-DES-013)" y "Producto: NVR 16 Canales... (SEGV-NVR16-050)" (CRUD-MOV/UI-LST esta ronda). |
| VIS-MOV-02 | Stock disponible visible antes de ingresar cantidad | ADMIN | "Stock disponible: N unidades" visible | ✅ PASS | Fase 1 R6 (2026-06-26): panel muestra "Stock físico / Reservado (órdenes pendientes) / Disponible para salida" con valores (ej. 104/0/104). |
| UI-MOV-01 | Selector de tipo: IN (Entrada) y OUT (Salida) disponibles | ADMIN | Ambas opciones en el `mat-select` | ✅ PASS | Fase 1 R6 (2026-06-26): "Entrada (IN)" y "Salida (OUT)" ambas presentes en el `mat-select` (verificado en CRUD-MOV/VAL-MOV esta ronda). |

### 3b. Validaciones (VAL)

| ID | Descripción | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| VAL-MOV-01 | Cantidad vacía → error | Campo vacío | Error "Cantidad requerida" | ✅ PASS | Fase 1 R6 (2026-06-26): Cantidad vacía + blur → "La cantidad es obligatoria." |
| VAL-MOV-02 | Cantidad = 0 → error | Valor 0 | Error "Debe ser mayor a 0" | ✅ PASS | Fase 1 R6 (2026-06-26): cantidad=0 (tecleo real) → "La cantidad debe ser al menos 1." |
| VAL-MOV-03 | Motivo vacío → error | Campo vacío | Error "Motivo requerido" | ✅ PASS | Fase 1 R6 (2026-06-26): Motivo vacío + blur → "El motivo es obligatorio." |
| VAL-MOV-04 | OUT con cantidad > stock disponible → error | availableStock = 5, cantidad = 10 | Error de validación; botón Guardar deshabilitado | ✅ PASS | Fase 1 R6 (2026-06-26): tipo OUT (HMAN-DES-013 disponible=104), cantidad=9999 → "Supera el stock disponible (104 unidades)." + "Registrar" `disabled:true`. |
| VAL-MOV-05 | IN sin límite de cantidad — cualquier cantidad positiva válida | Tipo IN, cantidad 9999 | Sin error de máximo | ✅ PASS | Fase 1 R6 (2026-06-26): tipo IN, cantidad=9999 → sin error "Supera…" (solo persiste error de motivo vacío). |
| VAL-MOV-06 | Motivo con > 300 caracteres → error | Texto largo | Error "Máximo 300 caracteres" | ✅ PASS | Fase 1 R6 (2026-06-26): `<textarea>` motivo con `maxlength="300"` confirmado vía DOM (prevención de exceso). |

### 3c. Reglas de negocio (RN)

| ID | Descripción | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| RN-MOV-01 | Al cambiar tipo de IN a OUT se aplica validación de máximo automáticamente | Tipo cambia a OUT | Validación máx=availableStock aplicada sin submit | ✅ PASS | Fase 1 R6 (2026-06-26): con cantidad=9999 fija, seleccionar OUT → "Supera el stock disponible (104 unidades)." aparece sin tocar Cantidad (VAL-MOV-04 esta ronda). |
| RN-MOV-02 | Al cambiar tipo de OUT a IN se quita la validación de máximo | Tipo cambia de OUT a IN | Validación de máximo desaparece | ✅ PASS | Fase 1 R6 (2026-06-26): con cantidad=9999 fija, OUT→IN → error "Supera el stock disponible" desaparece (VAL-MOV-05 esta ronda). |
| RN-MOV-03 | Movimiento OUT correcto → stock disminuye en la lista | ADMIN, OUT 3 unidades | Lista de productos recarga; stock = anterior − 3 | ✅ PASS | Fase 1 R6 (2026-06-26): OUT -3 sobre HMAN-DES-013, 107→104 (CRUD-MOV-02 esta ronda). |
| RN-MOV-04 | Movimiento IN correcto → stock aumenta en la lista | ADMIN, IN 5 unidades | Lista de productos recarga; stock = anterior + 5 | ✅ PASS | Fase 1 R6 (2026-06-26): IN +5 sobre HMAN-DES-013, 102→107 (CRUD-MOV-01 esta ronda). |

### 3d. CRUD Movimiento (CRUD)

| ID | Descripción | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| CRUD-MOV-01 | Registrar movimiento IN válido | Formulario completo, tipo IN | Snackbar verde "Movimiento registrado correctamente."; diálogo cierra | ✅ PASS | Fase 1 R6 (2026-06-26): IN cantidad=5, motivo "[QA] Entrada prueba CRUD-MOV-01 Ronda6" sobre HMAN-DES-013 → snackbar verde "Movimiento registrado correctamente.", diálogo cierra, stock **102→107**. Obs (no defecto): el botón Registrar requiere selección explícita del tipo en el `mat-select` aunque muestre "Entrada (IN)" por defecto. |
| CRUD-MOV-02 | Registrar movimiento OUT válido | Formulario completo, tipo OUT, cantidad ≤ availableStock | Snackbar verde; diálogo cierra | ✅ PASS | Fase 1 R6 (2026-06-26): OUT cantidad=3, motivo "[QA] Salida prueba CRUD-MOV-02 Ronda6" sobre HMAN-DES-013 → snackbar verde "Movimiento registrado correctamente.", diálogo cierra, stock **107→104**. |
| CRUD-MOV-03 | Cancelar / cerrar sin guardar | Formulario con datos | Diálogo cierra; stock sin cambios; lista no recarga | ✅ PASS | Fase 1 R6 (2026-06-26): cantidad=10, clic "Cancelar" → diálogo cierra, stock HMAN-DES-013 permanece en **104** (sin cambios). Nota: stock neto de HMAN-DES-013 tras MOV-01/02 = +2 (102→104), mutación QA documentada. |
...
| ID | Descripción | Rol | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| VIS-CAT-01 | Título "Categorías" visible | ADMIN | Encabezado presente | ✅ PASS | Fase 1 R6 (2026-06-26): breadcrumb "Inventario → Categorías" visible. |
| VIS-CAT-02 | Columnas tabla: Nombre, Descripción, Acciones | ADMIN | Todas las columnas visibles | ✅ PASS | Fase 1 R6 (2026-06-26): columnas "Nombre", "Descripción" + ícono `shopping_bag` (ver productos) por fila. |
| VIS-CAT-03 | `mat-progress-bar` visible durante carga | ADMIN | Barra en parte superior | ✅ PASS | Fase 1 R6 (2026-06-26): `@if (loading) { <mat-progress-bar> }` en template (code review). |
...
| ID | Descripción | Rol | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|---|
| BSRCH-CAT-01 | Buscar por nombre exacto | ADMIN | Categorías en BD | Filtra a la(s) categoría(s) coincidente(s) | ✅ PASS | Fase 1 R6 (2026-06-26): "Cat-Cy-47061" → 1 resultado exacto. |
| BSRCH-CAT-02 | Buscar en minúsculas (case insensitive) | ADMIN | Categoría con nombre en mayúsculas/mixto | Encuentra la categoría sin importar mayúsculas/minúsculas | ✅ PASS | Fase 1 R6 (2026-06-26): "cat-cy-47061" (minúsculas) → "Cat-Cy-47061". |
| BSRCH-CAT-03 | Buscar sin acento encuentra nombre acentuado (accent insensitive) | ADMIN | Categoría con acento (ej. "Climatización") | "climatizacion" encuentra "Climatización" — verificar `f_unaccent()` en `CategoryRepository` | ✅ PASS | Fase 1 R6 (2026-06-26): "construccion" (sin acento) → "Material de Construcción" (f_unaccent). |
| BSRCH-CAT-04 | Término sin resultados muestra estado vacío | ADMIN | Término inexistente | Ícono + 'Sin resultados para "…"' | ✅ PASS | Fase 1 R6 (2026-06-26): "zzzznoexiste" → 0 filas + empty-state "Sin resultados / Ningún registro coincide con los filtros aplicados." |
| BSRCH-CAT-05 | Limpiar campo de búsqueda restaura la lista completa | ADMIN | Campo con término activo | Lista restaura todas las categorías activas | ✅ PASS | Fase 1 R6 (2026-06-26): borrar el campo restaura la lista completa "1–20 of 273" (273 categorías activas, infladas por datos QA Cat-Cy — L33). |
| BSRCH-CAT-06 | Buscar con ñ — normalización bidireccional | ADMIN | Categoría con "ñ" en el nombre (si existe) o usar "Climatización"/"Construcción" | Búsqueda sin diacríticos encuentra el nombre con diacríticos y viceversa | ✅ PASS | Fase 1 R6 (2026-06-26): "límpíezá" (acentos añadidos) → "Limpieza Profesional" (sin acentos en BD) — normalización bidireccional confirmada. |

### 4c. RBAC en categorías (RBAC)

| ID | Descripción | Rol | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| RBAC-CAT-01 | Botón "Nueva categoría" visible | ADMIN | Visible | ✅ PASS | Fase 1 R6 (2026-06-26): `admin` — botón "Nueva categoría" presente (DOM + screenshot esquina sup. der.). |
| RBAC-CAT-02 | Botón "Nueva categoría" visible | MANAGER | Visible | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_manager` — chip MANAGER, botón "Nueva categoría" presente. |
| RBAC-CAT-03 | Botón "Nueva categoría" AUSENTE | WAREHOUSEMAN | Ausente del DOM | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_warehouse` — botón "Nueva categoría" AUSENTE del DOM. |
| RBAC-CAT-04 | Botón "Nueva categoría" AUSENTE | SALES | Ausente del DOM | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_sales` — botón "Nueva categoría" AUSENTE del DOM. |
| RBAC-CAT-05 | Click en fila (ADMIN) abre diálogo edición | ADMIN | Diálogo de edición abre | ✅ PASS | Fase 1 R6 (2026-06-26): `admin` — click fila "Cat-Cy-18832" abre "Editar categoría" con Nombre="Cat-Cy-18832"/Descripción="test", botones Desactivar/Cancelar/Guardar cambios. |
| RBAC-CAT-06 | Click en fila (WAREHOUSEMAN) NO abre diálogo | WAREHOUSEMAN | Sin acción; no abre diálogo | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_warehouse` — múltiples clicks en filas → ningún diálogo (overlayCount 0). |

### 4d. Botones e íconos de acción (UI)

| ID | Descripción | Rol | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|---|
| UI-CAT-01 | Ícono "Ver productos" en fila → navega a `/inventory/products?categoryId=X` | ADMIN | Lista cargada | Navega a productos filtrados por esa categoría | ✅ PASS | Fase 1 R6 (2026-06-26): clic shopping_bag de "Herramientas Manuales" → `/inventory/products?categoryId=6`, 2 productos todos de esa categoría (verificado en BSRCH-PROD-04 esta ronda). |
| UI-CAT-02 | Ícono "Editar" en fila → abre diálogo | ADMIN | Lista cargada | Diálogo de edición abre con datos precargados | ✅ PASS | Fase 1 R6 (2026-06-26): click en fila → "Editar categoría" con Nombre/Descripción precargados (CRUD-CAT/RBAC-CAT-05 esta ronda; sin ícono dedicado, click en fila completa). |
| UI-CAT-PAG-01 | Paginador visible cuando hay > pageSize categorías | BD con >20 categorías | Paginador activo | ✅ PASS | Fase 1 R6 (2026-06-26): "1 – 20 of 273", "Next page" habilitado, selector page-size presente. |
| UI-CAT-PAG-02 | Cambiar tamaño de página (10/20/50) recarga la lista con el nuevo tamaño | Paginador visible, >50 categorías | Selector "Items por página" cambia a 10/50; tabla muestra el número correcto de filas; vuelve a página 0 | ✅ PASS | Fase 1 R6 (2026-06-26): en página 2 ("21-40 of 273"), cambiar de 20 a 50 → "1 – 50 of 273" (página 0), 50 filas. |

### 4e. CRUD Categorías (CRUD)

| ID | Descripción | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| CRUD-CAT-01 | Crear categoría con datos válidos | Formulario completo | Snackbar verde "Categoría creada."; diálogo cierra; lista recarga | ✅ PASS | Fase 1 R6 (2026-06-26): creada "[QA] Cat Ronda6 Test" / descripción QA → snackbar verde `rgb(46,125,50)` "Categoría creada correctamente.", diálogo cierra, búsqueda la muestra. |
| CRUD-CAT-02 | Editar nombre de categoría | Categoría existente | Snackbar verde "Categoría actualizada."; lista refleja cambio | ✅ PASS | Fase 1 R6 (2026-06-26): "[QA] Cat Ronda6 Test" → "[QA] Cat Ronda6 Test Editada" (tecleo real) → snackbar verde "Categoría actualizada correctamente.", lista refleja el nuevo nombre. |
| CRUD-CAT-03 | Desactivar categoría abre diálogo de confirmación | ADMIN, categoría activa | Modal con texto de confirmación | ✅ PASS | Fase 1 R6 (2026-06-26): clic "Desactivar" → modal "Desactivar categoría" con texto "¿Deseas desactivar la categoría \"[QA] Cat Ronda6 Test Editada\"? Esta acción ocultará la categoría del sistema." + botones Cancelar/Desactivar. |
| CRUD-CAT-04 | Confirmar desactivación | ADMIN | Snackbar verde; categoría ya no aparece | ✅ PASS | Fase 1 R6 (2026-06-26): confirmar → snackbar verde "Categoría desactivada.", la categoría QA desaparece (empty-state). Limpieza L33: categoría QA desactivada. |
| CRUD-CAT-05 | Intentar desactivar categoría con productos activos | Categoría con productos | Snackbar rojo con mensaje del backend | ✅ PASS | Fase 1 R6 (2026-06-26): "Desactivar"→confirmar sobre "Herramientas Manuales" → snackbar rojo `rgb(198,40,40)` (#C62828) `snackbar-error` "No se puede desactivar la categoría: tiene productos activos asignados. Reasigne o desactive los productos antes de continuar." Categoría intacta (422, sin mutación). |

### 4f. Validaciones formulario de categoría (VAL)

| ID | Descripción | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| VAL-CAT-01 | Nombre vacío → error inline | Campo vacío | Error "Nombre requerido" | ✅ PASS | Fase 1 R6 (2026-06-26): "Editar categoría" Cat-Cy-18832, vaciar Nombre + Tab → "El nombre es obligatorio." + "Guardar cambios" `disabled:true`. |
| VAL-CAT-02 | Nombre duplicado → error del backend | Nombre ya existe | Snackbar rojo con mensaje 409/422 | ✅ PASS | Fase 1 R6 (2026-06-26): editar Cat-Cy-18832 → Nombre "Herramientas Manuales" → Guardar → snackbar rojo `rgb(198,40,40)` `snackbar-error` "Ya existe otra categoría con el nombre 'Herramientas Manuales'.", diálogo permanece abierto. Cancelado (sin mutación). |
| VAL-CAT-03 | Botón Guardar deshabilitado al cargar formulario de edición | Sin modificaciones | `disabled:true` (form.dirty = false) | ✅ PASS | Fase 1 R6 (2026-06-26): abrir "Editar categoría" Cat-Cy-18832 sin modificar → "Guardar cambios" `disabled:true`. |
| VAL-CAT-04 | Botón Guardar se activa al modificar un campo | Modificar nombre | `disabled:false` | ✅ PASS | Fase 1 R6 (2026-06-26): teclear en Nombre (tecleo real) → "Guardar cambios" `disabled:false`. |
...
| ID | Descripción | Rol | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| RBAC-LST-01 | Ícono "Registrar movimiento" visible | ADMIN | Visible en cada fila | ✅ PASS | Fase 1 R6 (2026-06-26): ícono `add_circle` presente en cada fila (2 filas low-stock actuales → 2 íconos); columna "Costo unit." presente (ADMIN). |
| RBAC-LST-02 | Ícono "Registrar movimiento" visible | WAREHOUSEMAN | Visible en cada fila | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_warehouse` — `add_circle` en cada fila (2/2). |
| RBAC-LST-03 | Ícono "Registrar movimiento" AUSENTE | SALES | Columna acciones ausente | ✅ PASS | Fase 1 R6 (2026-06-26): login `qa_sales` — headers Nivel/SKU/Nombre/Categoría/Proveedor/Físico/Reservado/Disponible/Mínimo/Déficit, sin columna "Acciones" ni "Costo unit.", 0 íconos de movimiento, sin snackbar de error. |

### 5c. Botones e íconos de acción (UI)

| ID | Descripción | Rol | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|---|
| UI-LST-01 | Ícono movimiento → abre `MovementDialog` correctamente | ADMIN | Lista con productos bajo stock | Diálogo abre con datos del producto | ✅ PASS | Fase 1 R6 (2026-06-26): clic `add_circle` de SEGV-NVR16-050 → "Registrar movimiento de stock" con "Producto: NVR 16 Canales 4K PoE 16 Puertos (SEGV-NVR16-050)", Stock físico=1, Disponible=1, tipo "Entrada (IN)" preseleccionado. |
| UI-LST-02 | Movimiento IN desde stock bajo → lista recarga | ADMIN | MovementDialog abierto, tipo IN | Snackbar verde; lista actualizada | ✅ PASS | Fase 1 R6 (2026-06-26): IN +5 en SEGV-NVR16-050 (1→6) → snackbar verde "Movimiento registrado correctamente."; lista recargó: salió de bajo stock (2→1 filas, ya supera el mínimo 4). Obs: el `MovementDialog` a veces se renderiza semitransparente durante la animación de apertura (verificar en VIS); funcional. |

### 5d. Reglas de negocio (RN)

| ID | Descripción | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| RN-LST-01 | Productos ordenados por déficit descendente (mayor déficit primero) | Lista cargada con varios productos | Producto con mayor déficit (min-available) en primera fila | ✅ PASS | Fase 1 R6 (2026-06-26): estado de 2 filas durante UI-LST → SEGV-NVR16-050 (Déficit +3) en 1ª fila antes de HELEC-SIE-048 (Déficit 0) — orden descendente. |
| RN-LST-02 | Déficit = minimumStock − availableStock | Producto con min=10, available=3 | Columna "Déficit" muestra 7 | ✅ PASS | Fase 1 R6 (2026-06-26): HELEC-SIE-048 min=8, available=8 → Déficit=0 (8-8); SEGV-NVR16-050 min=4, available=1 → Déficit=+3 (4-1). Fórmula confirmada. |
| RN-LST-03 | Producto con reservas y stock ≥ mínimo → severidad "Por reservas" | Producto con reservedStock>0, currentStock≥min | Etiqueta "Por reservas" en columna severidad | ✅ PASS | Fase 1 R6 (2026-06-26): HELEC-SIE-048 (Físico=8=Mínimo=8, Reservado=0, Disponible=8) → severidad "Por reservas" + chip. Nota: `getSeverity()` asigna "reservas" cuando `currentStock >= minimumStock` (reservedStock=0 aquí — condición más amplia que la descrita, consistente con el código; observación, no bug). |
...
| ID | Descripción | Precondición | Resultado esperado | Estado | Notas |
|---|---|---|---|---|---|
| ERR-01 | Snackbar éxito: fondo verde `#2E7D32` | Crear/editar producto o categoría | Color correcto; clase `snackbar-success` | ✅ PASS | Fase 1 R6 (2026-06-26): snackbar verde `rgb(46,125,50)` (#2E7D32) clase `snackbar-success` en crear/editar/desactivar producto, movimientos y categorías (CRUD-PDET/MOV/CAT esta ronda). |
| ERR-02 | Snackbar error: fondo rojo `#C62828` | Error del backend | Color correcto; clase `snackbar-error` | ✅ PASS | Fase 1 R6 (2026-06-26): snackbar rojo `rgb(198,40,40)` (#C62828) clase `snackbar-error` en CRUD-CAT-05, VAL-CAT-02, VAL-PDET-09 esta ronda. |
| ERR-03 | Mensaje específico del backend en snackbar (no genérico) | Backend rechaza con 409/422 | Texto descriptivo del backend visible | ✅ PASS | Fase 1 R6 (2026-06-26): mensajes específicos verificados: "Ya existe otra categoría con el nombre 'Herramientas Manuales'." (VAL-CAT-02), "El SKU 'LUBR-ACE20-035' ya está en uso por otro producto." (VAL-PDET-09), "No se puede desactivar la categoría: tiene productos activos asignados..." (CRUD-CAT-05). |
| ERR-04 | Error al registrar movimiento OUT > stock → mensaje en el diálogo (no snackbar) | OUT cantidad > disponible | Mensaje de error inline en el diálogo; diálogo NO se cierra | ✅ PASS | Fase 1 R6 (2026-06-26): OUT=9999 sobre HMAN-DES-013 (disponible=104) → error inline "Supera el stock disponible (104 unidades)." bajo Cantidad, "Registrar" deshabilitado, diálogo permanece abierto (VAL-MOV-04 esta ronda). |
...
| ID | Descripción | Resultado esperado | Estado | Notas |
|---|---|---|---|---|
| VIS-GEN-01 | Sidebar colapsado al entrar a cualquier pantalla del módulo | Sidebar en modo íconos al navegar | ✅ PASS | Fase 1 R6 (2026-06-26): sidebar en modo íconos (solo íconos, sin etiquetas) en `/inventory/products`, `/categories` y `/low-stock` — confirmado en screenshots de toda la ronda. |
| VIS-GEN-02 | Breadcrumb correcto: "Inventario → Productos" / "→ Categorías" / "→ Stock bajo" | Breadcrumb en topbar correcto | ✅ PASS | Fase 1 R6 (2026-06-26): "Inventario → Productos", "Inventario → Categorías", "Inventario → Bajo stock" correctos en topbar. |
| VIS-GEN-03 | Botones primarios con color de marca `#6B3C6B` | "Nuevo producto", "Nueva categoría", "Guardar" con color correcto | ✅ PASS | Fase 1 R6 (2026-06-26): "Nuevo producto" → `getComputedStyle` bg `rgb(107,60,107)` (#6B3C6B), texto `rgb(255,255,255)` (blanco). |
| VIS-GEN-04 | Botones destructivos con color `warn` (rojo) | "Desactivar" en rojo | ✅ PASS | Fase 1 R6 (2026-06-26): botón "Desactivar" → `getComputedStyle` color `rgb(244,67,54)` (#F44336, warn rojo). |
| VIS-GEN-05 | Diálogos de confirmación son modales (click fuera no cierra) | Click backdrop → diálogo permanece | ✅ PASS | Fase 1 R6 (2026-06-26, BUG-INV-14 sigue corregido): "Editar producto" → click backdrop → permanece abierto; ESC → permanece abierto (`disableClose:true`). |
| VIS-GEN-06 | Header de tabla con fondo `#F2E4F2` y texto `#6B3C6B` | Colores de marca correctos en todas las tablas | ✅ PASS | Fase 1 R6 (2026-06-26, BUG-INV-07 sigue corregido): `getComputedStyle('.mat-mdc-header-cell')` → bg `rgb(242,228,242)` (#F2E4F2), color `rgb(107,60,107)` (#6B3C6B). |
| VIS-GEN-07 | Texto largo en celdas se trunca con `…` y tooltip con valor completo | Nombre/descripción larga | Truncado + tooltip | ✅ PASS | Fase 1 R6 (2026-06-26): celdas con `text-overflow:ellipsis` + `overflow:hidden` (truncado) y `matTooltip`/`ng-reflect-message` presente (tooltip con valor completo). |
