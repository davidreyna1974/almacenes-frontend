...
| **Siguiente acción** | **FASE 1 — Inventario**: ejecutar casos de prueba bajo Protocolo 4 fases. Congelamiento verificado (git limpio, backend 200, frontend 200, 4 usuarios QA autentican). Iniciar con categoría **SEC** (6 casos). Ver `CONTEXTO_SESION_SIGUIENTE.md` para instrucciones detalladas. Actualizar `estado_sesion_activa.md` tras cada categoría completada. |
