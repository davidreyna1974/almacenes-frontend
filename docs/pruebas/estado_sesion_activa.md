...
| **Siguiente acción** | **FASE 1 — Inventario (Ronda 6) EN CURSO**: congelamiento verificado 2026-06-26 (git limpio develop 0/0, backend 200, frontend 200, 4 usuarios QA autentican 200). **SEC 5/5 ✅**, **RBAC 28/28 ✅**, **CRUD 13/13 ✅**, **VAL 25/25 ✅**, **BSRCH 14/14 ✅**, **UI 18/18 ✅**, **FLOW 8 N/A ✅**, **RN 7/7 ✅**, **ERR 4/4 ✅**, **EMPTY 2/2 ✅**, **VIS 24/24 ✅**, **CYBER 21/21 ✅ (+1 N/A)** completadas. **🎉 FASE 1 INVENTARIO COMPLETA: 152 PASS · 0 FAIL · 0 PENDIENTE · 46 N/A · 0 bugs funcionales.** Código congelado de principio a fin. **Siguiente paso: FASE 4 — Certificación** (`ng build` + `ng test --no-watch --coverage` ≥70% + `mvn test` + commit `chore(qa)`), ya que Fase 1 cerró con 0 bugs → no hay Fase 2/3 que ejecutar. Observación a reconciliar (no defecto): VIS-LST-02 "Por reservas" = Info-azul #1565C0 vs "amarillo" del doc. Actualizar `estado_sesion_activa.md` tras cada categoría completada. Regla: bug encontrado → ⚠️ ABIERTO, NO corregir, terminar fase. Notas técnicas: (1) login UI sufre autofill de Chrome ("admin") — usar set-value JS + dispatch input/blur + click submit para roles QA; (2) seleccionar opciones de `mat-select` requiere click UI real (JS .click en mat-option no dispara la selección de Angular Material); (3) tecleo real necesario para activar botones guard `form.dirty` (L25). |
